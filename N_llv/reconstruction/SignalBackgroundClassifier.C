...
Int_t ClassifySingal(TClonesArray* branchParticle, iFinalStates* iFSTrue, TLorentzVector* NTrue, Int_t NPID) {
    ...
    Int_t iLepTrue2 = 99999;  // index for lepton from N
    Int_t foundLep = 0;
    ...
    for (Int_t iN = 0; iN < nParticles; iN++) {
        ...
        if (abs(particle->PID) == NPID) {  // find HNL
            ...
            for (Int_t i = 0; i < nParticles; i++) {  // find lepton (electron or muon only)
                particle0 = (GenParticle*)branchParticle->At(i);
                particle0M = (GenParticle*)branchParticle->At(i);
                if (particle0->M1 == -1) continue;
                if ((abs(particle0->PID) == 11 || abs(particle0->PID) == 13)) {
                    while (true) {
                        Int_t Mindex = particle0M->M1;
                        particle0M = (GenParticle*)branchParticle->At(Mindex);
                        if (particle0M->M1 == -1) break;
                        if (abs(particle0M->PID) == 24 && particle0M->M1 == iNTrue) {
                            foundLep += 1;
                            iLepTrue2 = i;
                            // cout << " text:\n ";
                            break;
            ...
            for (Int_t i = 0; i < nParticles; i++) {  // find lepton (electron or muon only)
                ...
                particle0M = (GenParticle*)branchParticle->At(i);
                if (particle0->M1 == -1) continue;
                if ((abs(particle0->PID) == 12 || abs(particle0->PID) == 14)) {
                    while (true) {
                        Int_t Mindex = particle0M->M1;
                        particle0M = (GenParticle*)branchParticle->At(Mindex);
                        if (particle0M->M1 == -1) break;
            ...
            // if ((abs(particle0->PID) == 12 || abs(particle0->PID) == 14) && abs(particle0M->PID) == 24 && particle0M->M1 == iNTrue) {
            // foundNu += 1;
            // iNuTrue = i;
            // break;
            //}
            //}
            if (foundNu >= 1 && foundLep >= 2) {
                // if (foundLep >= 2) {
                iFinalStates iFSTrue_;

                GenParticle* lepParticle = (GenParticle*)branchParticle->At(iLepTrue);
                GenParticle* lepParticle2 = (GenParticle*)branchParticle->At(iLepTrue2);
                GenParticle* NuParticle = (GenParticle*)branchParticle->At(iNuTrue);
                GenParticle* NParticle = (GenParticle*)branchParticle->At(iNTrue);

                TLorentzVector lepTrue_, lepTrue2_, Q1True_, Q2True_, NTrue_, NuTrue_;
                lepTrue_.SetPtEtaPhiM(lepParticle->PT, lepParticle->Eta, lepParticle->Phi, 0);
                lepTrue2_.SetPtEtaPhiM(lepParticle2->PT, lepParticle2->Eta, lepParticle2->Phi, 0);
                NTrue_.SetPtEtaPhiE(NParticle->PT, NParticle->Eta, NParticle->Phi, NParticle->E);
                NuTrue_.SetPtEtaPhiE(NuParticle->PT, NuParticle->Eta, NuParticle->Phi, NuParticle->E);

                iFSTrue_.iLeps.push_back(lepTrue_);
                iFSTrue_.iLeps.push_back(lepTrue2_);
                iFSTrue_.iNu.push_back(NuTrue_);
                // cout << " NuTrue_: " << NuTrue_.Pz() << "\n";
